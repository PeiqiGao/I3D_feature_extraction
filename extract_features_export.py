...
def load_frame(frame_file, resize=False):
    ...
    if resize:
        data = data.resize((320, 320), Image.ANTIALIAS)
...
def load_flow_batch(frames_dir, flow_x_files, flow_y_files, 
    ...
    if resize:
        batch_data = np.zeros(frame_indices.shape + (320,320,2))
    else:
...
def run(mode='rgb', load_model='', sample_mode='oversample', frequency=20,
    ...
    torch.onnx.export(i3d,               # model being run
                    ...
                    "i3d_rgb_imagenet.onnx",   # where to save the model (can be a file or file-like object)
