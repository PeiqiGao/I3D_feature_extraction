...
from pytorch_i3d_torch import InceptionI3d
...
def load_frame(frame_file, resize=False):
    ...
    if resize:
        data = data.resize((344, 256), Image.ANTIALIAS)
...
def load_rgb_batch(frames_dir, rgb_files, 
    ...
    if resize:
        # batch_data = np.zeros(frame_indices.shape + (320,320,3))
        batch_data = np.zeros(frame_indices.shape+(256,344,3))
    else:
        batch_data = np.zeros(frame_indices.shape + (256,340,3))

    for i in range(frame_indices.shape[0]):
        for j in range(frame_indices.shape[1]):
            if frame_indices[i][j]<0:
                frame_indices[i][j] = 0
            elif frame_indices[i][j] > len(rgb_files)-2:
                frame_indices[i][j] = len(rgb_files)-2
...
def load_flow_batch(frames_dir, flow_x_files, flow_y_files, 
    ...
    if resize:
        batch_data = np.zeros(frame_indices.shape + (256,344,2))
    else:
    ...
    for i in range(frame_indices.shape[0]):
        for j in range(frame_indices.shape[1]):
            ...
            if frame_indices[i][j]<0:
                frame_indices[i][j] = 0
            elif frame_indices[i][j] > len(flow_x_files)-1:
                frame_indices[i][j] = len(flow_x_files)-1
...
def run(mode='rgb', load_model='', sample_mode='oversample', frequency=20,
    ...
    video_names = [i for i in os.listdir(input_dir) if i[0] == 'r']
    for video_name in video_names:
        ...
        clipped_length = frame_cnt+1
        
        if(mode == 'flow'):
            clipped_length = frame_cnt+2
        
        frame_indices = [] # Frames to chunks
        for i in range(clipped_length // frequency):
            frame_indices.append(
                [j for j in range(i * frequency -4, i * frequency + 12)])
        ...
        for batch_id in range(batch_num):
            ...
            if sample_mode == 'oversample':
                ...
                # assert(batch_data.shape[-2]==320)
                # assert(batch_data.shape[-3]==320)
